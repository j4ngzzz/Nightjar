...
Nightjar is a verification orchestrator for Python. You write a behavioral spec. Nightjar runs six stages of automated checking — from syntax validation to mathematical proof — and tells you whether your code satisfies the spec for all inputs, not just the ones you tested.

Under the hood, Nightjar coordinates Hypothesis (property-based testing), CrossHair (symbolic execution), and Dafny (formal verification). You don't need to learn any of these tools. You write a `.card.md` spec. Nightjar picks the right verification strategy based on complexity and short-circuits at the first failure.
...
Using the same property-based testing methodology that Nightjar automates, we tested 34 popular Python packages and found 74 confirmed bugs — zero false positives. Every finding runs in a standalone reproduction script.

These are the kinds of edge cases that property-based testing catches and traditional unit tests miss:
...
The difference: Nightjar coordinates all three approaches (type checking + pattern scanning + property testing + formal proof) in one pipeline, cheapest-first. You don't configure Hypothesis strategies, write CrossHair contracts, or learn Dafny syntax. You write what the code should do. Nightjar proves whether it does.

---

## How it works

Nightjar orchestrates multiple verification tools so you don't have to learn them individually. It routes each function to the cheapest verification method that can prove it correct:
